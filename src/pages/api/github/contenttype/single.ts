...
export default async (
  ...
  try {
    ...
    const { siteId, contentTypeId } = req.query
    if (!siteId) throw new Error('Please provide a siteId')
    if (!contentTypeId) throw new Error('Please provide a contentTypeId')

    // First we need to get the current data of the file to check for
    // concurrent updates
    const githubFilename = `${contentFolder}/${contentTypeId}/${contentFile}`
